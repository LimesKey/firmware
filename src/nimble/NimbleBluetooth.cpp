...
// Number of FromRadio onRead callbacks currently executing in the NimBLE task, so deinit()
// can wait for one to exit before BLEDevice::deinit(true) frees the characteristic it is
// about to setValue() into (a late write there corrupts the heap).
//
// This is a backstop for deinit()'s disconnect drain, not a replacement: that drain waits
// for onDisconnect, which is dispatched on the same NimBLE task as onRead and so normally
// implies the read already returned. But it only runs when a connection handle is tracked,
// and its 2s cap can expire (a phone that walked out of range), after which nothing else
// would wait for the in-flight read.
...
class NimbleBluetoothFromRadioCallback : public BLECharacteristicCallbacks
{
    void onRead(BLECharacteristic *pCharacteristic) override
    {
        ...
        // Register as in-flight *before* checking isDeInit so deinit()'s drain can never
...
void NimbleBluetooth::deinit()
{
    ...
    // Backstop for the disconnect drain above, which is skipped when no connection handle is
    // tracked and gives up after 2s. bleDraining has already released the poll loop; wait
    // (bounded) for the read to actually leave the callback, so it can never setValue() into a
    // characteristic BLEDevice::deinit() has freed. isDeInit is set by now, so a read that is
    // still between the loop and the write bails on its own.
